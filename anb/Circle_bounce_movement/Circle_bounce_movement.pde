...
float update(float value,float increment) // update positon of circle
{
...
float check(float value, float delta, float lower_limit, float upper_limit)
{
  ...
  if(value > upper_limit || value < lower_limit)  //if the curren position of the circle is...
  {
    return(delta * (-1)); //smaller equal the screen with set it to zero
  }
  return(delta);
}
